...
<div style= "text-align: justify;margin : 25px 0"> Le but ici était de ne garder que la valeur de la composante verte du logo. Ce qui nous donne donc... un logo tout vert.</div>
...
<div style= "text-align: justify;margin : 25px 0"> Pour obtenir un logo noir et blanc nous avons, pour chacun des pixels qui composent l'image, calculé la moyenne des valeurs de ses composantes rouge, verte et bleue. Cette moyenne était ensuite affectée à chacune des composantes du pixel. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Ici, il nous a suffit d'inverser la valeur de chaque composante rouge, verte et bleue de chacun des pixels qui composent notre image (1 - valeur)</div>
...
<div style= "text-align: justify;margin : 25px 0"> L'exercice suivant consistait donc à créer un dégradé de couleur (ici du noir vers le blanc). Pour faire cela, il a fallu parcourir l'image de gauche à droite et d'éclaircir les pixels en fonction de leur position dans l'image. plus le pixel est proche du bord gauche, plus il est sombre et plus il est proche du bord droit, plus il est clair. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Pour avoir ce rendu "bruité" de notre logo, il a fallu utiliser pour la première fois dans ce workshop des valeurs aléatoires. En effet, pour obtenir cette transformation, il nous a fallu parcourir l'image et changer la couleur de pixels aléatoires avec des valeurs aléatoires. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Pour faire tourner notre image à 90°, il nous a fallu reporter chaque pixel vers une nouvelle coordonnée calculée grâce à une formule générique qui fonctionne pour chacun des pixels de l'image. Dans cet exercice, la difficulté a été d'adapter les dimensions de la nouvelle image etant donné que l'image utilisée est un rectangle. La largeur devient hauteur et inversement. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Ce nouvel effet consiste à donner à chacun des pixels de l'image la valeur de composante rouge d'un pixel un peu à sa gauche et la valeur de composante bleue d'un pixel un peu à sa droite dans l'image ( en faissant bien attention de ne pas chercher une valeur d'un pixel qui est en dehors de l'image ). La valeur de la composante verte de chaque pixel, elle, ne change pas. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Eclaircir une photo, ou l'assombrir est un peu plus compliqué que de simplement augmenter ou diminuer les valeurs des composantes de couleur des pixels. En effet, additionner ou soustraire des valeurs à ces composantes donnera un rendu où les blancs et les noirs sont trop agressifs et peu agréables à l'oeil. Nous avons donc utilisé les puissances pour augmenter ou diminuer les valeurs de composantes des pixels de l'image. Ainsi, plus la puissance est élevée, plus l'image sera sombre et inversement. </div>
...
<div style= "text-align: justify;margin : 25px 0"> Créer un disque sur une image noire a été un exercice plutôt simple pour nous. En effet il nous a suffit de trouver l'équation du cercle que nous souhaitions. En parcourant l'image, si un pixel se trouve être à une coordonnée à l'intérieur du cercle, alors il devient blanc, sinon il ne change pas.</div>
...
<div style= "text-align: justify;margin : 25px 0;"> Pour dessiner cette rosace, nous avons réutilisé notre programme permettant de tracer un cercle que nous avons répété 7 fois (ou plus en fonction de la complexité de la rosace que l'on souhaite). Pour commencer, nous avons tracé le cercle au centre de l'image. Ensuite, nous nous sommes servi de ce dernier comme cercle trigonométrique. Ainsi nous avons pu placer chacun des autres cercles dont le centre à été calculé tous les π/3, ce qui nous donne cette belle rosace. (Si l'on trace plus de cercles, alors il faut choisir des intervales proportionnellement moins espacés sur le cercle trigonométrique). </div>
...
<div style= "text-align: justify;margin:25px 0;"> Afin d'obtenir cet effet de glitch, il nous a fallu de noiveau utiliser des valeurs alétaoires. Dans un premier temps, nous avons obtenu un résultat plutôt joli mais qui n'était pas du tout celui attendu. Effectivement, cela était dû à un mauvais calcul de coordonnées et à la mauvaise utilisation de l'aléatoire. Le bon algorithme que nous avons finalement appliqué consistait en prendre un rectancle de pixels de taille aléatoire à une endroit aléatoire dans l'image et de l'échanger avec un autre rectangle de pixels de même taille mais placé à un autre endroit (alétoire lui aussi) dans cette même image.</div>
...
<div style= "text-align: justify;margin:25px 0;"> Cette fractale était notre objectif au début de ce workshop, au vu de sa belle forme assez complexe... Et oui nous avons utilisé les nombres complexes pour obtenir ce résultat ! En effet à l'aide d'une librairie C++ nous avons manipulé des nombres complexes sur lesquels nous avons fait opérations. Ainsi, pour chaque pixel, en fonction de sa postion (x,y), sa couleur est déterminée. On le voit bien avec les nuances de gris qui entourent notre fractale. </div>
...
<div style= "text-align: justify;margin:25px 0;"> Le principe de la convolution consiste, pour chaque pixel, de regarder tous les pixels qui l'entourent dans un carré de pixels. En effectuant un calcul liant chacunes des composantes des pixels composant le carré, on affecte une nouvelle valeur au pixel central.
